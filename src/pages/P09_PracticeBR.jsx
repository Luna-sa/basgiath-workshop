...
export default function P09_PracticeBR() {
  ...
  const submitted = !!taskSubmissions[9]
    ...
    <PageShell pageIndex={9}>
      <div className="space-y-5">
        ...
        <div className="p-5 border border-border border-l-[3px] border-l-qa-teal bg-surface/50 text-left">
          ...
          <p className="text-sm text-text-body mb-3">Найди баг в Nexus Academy и оформи отчёт:</p>
          ...
          <div className="p-3 bg-black border border-border mb-3">
            <code className="font-mono text-[12px] text-qa-teal">/bug-report В Nexus Academy можно залогиниться без пароля</code>
          </div>
          <p className="text-xs text-text-secondary mb-2">Ищи баги в:</p>
          <div className="grid grid-cols-2 gap-1.5">
            {['Логин без данных', 'Пароль видно всем', 'Cancel = Submit', 'Удаление без подтверждения', 'Даты из будущего', 'Форматы дат разные'].map((h, i) => (
              <div key={i} className="text-[11px] text-text-dim p-1.5 border border-border bg-black/30">› {h}</div>
            ))}
          </div>
          <div className="mt-3 p-3 border bg-surface/30" style={{ borderColor: persona.accentBorder }}>
            <p className="text-xs text-text-secondary">
              <span className="font-mono text-[11px] uppercase" style={{ color: persona.accent }}>Подход</span>{' '}— {persona.approach.bugHunting}
            </p>
          </div>
        </div>
        ...
        {!started && !submitted ? (
          ...
          <div className="text-center p-5 border border-forest/30 bg-forest/[0.05]"><p className="text-forest font-display">{persona.voice.successBig}</p></div>
        ) : showReview ? (
          <SubmissionReview type="bug-report" pageIndex={9} roundId="round-2" onComplete={() => completePage(9)} />
        ) : (
          <div className="space-y-3">
            <button onClick={() => setShowReview(true)} className="w-full px-6 py-4 text-black font-body text-[14px] font-semibold rounded-[2px] transition-all cursor-pointer" style={{ backgroundColor: persona.accent }}>Вставить баг-репорт на AI-ревью</button>
            <button onClick={() => { useWorkshopStore.getState().submitTask(9, { type: 'self-report' }); completePage(9) }} className="w-full px-6 py-3 text-text-dim text-[13px] border border-border hover:border-text-dim transition-colors cursor-pointer">Пропустить ревью</button>
          </div>
        )}
      </div>
    </PageShell>
