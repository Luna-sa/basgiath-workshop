    ...
    <PageShell pageIndex={12}>
      <div className="space-y-6">
        ...
        <div className="p-6 border border-border border-l-[3px] border-l-qa-teal bg-surface/50 text-left">
          ...
          <p className="text-sm text-text-body mb-2">Открой <code className="font-mono text-ember text-xs bg-black/50 px-1">auth.js</code> из тестового проекта. Найди баг и запусти:</p>
          <a href="/sample-project/auth.js" target="_blank" className="inline-block mb-3 font-mono text-[12px] text-qa-teal border border-qa-teal/20 px-3 py-1.5 hover:bg-qa-teal/10 transition-colors">↗ Открыть auth.js</a>
          <div className="p-3 bg-black border border-border font-mono text-sm text-qa-teal mb-4">/bug-report [опиши найденный баг]</div>
          <p className="text-xs text-text-dim">Подсказка: баги с валидацией email, паролями, edge cases.</p>
        </div>
        {!started && !submitted ? (
