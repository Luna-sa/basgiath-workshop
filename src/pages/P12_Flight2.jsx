      ...
      <div className="space-y-5">
        ...
        <div className="p-5 border border-border border-l-[3px] border-l-qa-teal bg-surface/50 text-left">
          ...
          <div className="p-3 bg-black border border-border mb-3">
            <code className="font-mono text-[12px] text-qa-teal">/bug-report В Nexus Academy можно залогиниться без пароля — пустые поля проходят аутентификацию</code>
          </div>
        ...
        {!started && !submitted ? (
          ...
          <div className="text-center p-5 border border-forest/30 bg-forest/[0.05]">
            <p className="text-forest font-display">{persona.voice.successBig}</p>
          </div>
        ) : showReview ? (
          <SubmissionReview
            type="bug-report"
          ...
          <div className="space-y-3">
            <button onClick={() => setShowReview(true)}
              ...
              style={{ backgroundColor: persona.accent }}>
              Вставить баг-репорт на AI-ревью
            </button>
            <button onClick={() => {
              useWorkshopStore.getState().submitTask(12, { type: 'self-report', durationSeconds: 420 - timeLeft })
              completePage(12)
            }}
