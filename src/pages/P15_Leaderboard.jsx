          ...
          <div className="relative overflow-hidden border p-8" style={{ borderColor: persona.accentBorder, backgroundColor: persona.accentLight }}>
            {myChar?.image && (
              <div className="absolute inset-0 opacity-10">
                <img src={myChar.image} alt="" className="w-full h-full object-cover" />
              </div>
            )}
            <div className="relative">
              ...
              <div className="font-mono text-[11px] tracking-[2px] uppercase mt-1" style={{ color: persona.accent }}>{myChar?.title}</div>
            </div>
          </div>
        </div>
      ) : (
            ...
            return (
              <div key={s.id} className={`flex items-center gap-3 p-3 border ${isMe ? '' : 'border-border bg-surface/30'}`}
                ...
                <span className={`font-mono text-lg font-bold w-8 text-center ${i < 3 ? '' : 'text-text-dim'}`}
                  style={i < 3 ? { color: persona.accent } : {}}>
                  {i + 1}
                </span>
                {/* Character avatar */}
                {ch?.image ? (
                  <div className="w-8 h-8 rounded-full overflow-hidden shrink-0">
                    <img src={ch.image} alt="" className="w-full h-full object-cover object-top" />
                  </div>
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="font-mono text-[10px] text-text-dim">{ch?.title || ''}</div>
                </div>
